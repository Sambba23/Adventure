...
class Area(var name: String, var description: String, val useTools: Boolean = false):
  ...
  def fullDescription =
    val exitList = "\n\nExits available: " + this.neighbors.keys.filterNot(n => n == "secret").filterNot(n => n == "the hallway where the spurgus were fighting").filterNot(n => n == "jail").mkString(" ")
    if this.allItems.isEmpty then
      this.description + exitList
    else
      this.description + "\n" + "You see here: " + this.allItems.keys.mkString(" ") + "\n" + exitList
